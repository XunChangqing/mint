...
/ {
    ...
    amba {
        ...
        mhu@281c0000 {
            ...
            reg = <0x00000000 0x281c0000 0x00000000 0x00001000>;
		};
	};

	chosen {
		stdout-path = "/pl011@9000000";
		rng-seed = <0xdb0c1dc7 0x1468f1e 0xc302b153 0x1fd45aab 0xb1fa4bf7 0x45cbf4bf 0x38cea20c 0x2682a8a0>;
		kaslr-seed = <0x7a348639 0xc3aa25e0>;
	};
};
"""

class TestXdtMethods(unittest.TestCase):
    def test(self):
        dt = xdt.device_populate(TEST_VIRT_DTS)
        self.assertEqual(len(dt.cpus), 2)
        self.assertEqual(dt.cpus[0].id, 0)
        self.assertEqual(dt.cpus[1].id, 1)
        self.assertEqual(len(dt.memories), 1)
        self.assertEqual(dt.memories[0].start, 0x40000000)
        self.assertEqual(dt.memories[0].size, 0x8000000)
        self.assertEqual(len(dt.uarts), 2)
        self.assertEqual(dt.uarts[0].name, 'pl011')
        self.assertEqual(dt.uarts[1].name, 'dw16550')
        self.assertEqual(dt.chosen.stdout_name, 'pl011')
        self.assertEqual(dt.chosen.stdout_addr, 0x9000000)
