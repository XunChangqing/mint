...
#define NVME_CQ_ALLOCATION ALIGN(NVME_CQ_SIZE(NVME_Q_DEPTH), ARCH_DMA_MINALIGN)
...
static int nvme_wait_csts(struct nvme_dev *dev, u32 mask, u32 val) {
  ...
  while (get_timer(start) < timeout) {
    if ((readl(&dev->bar->csts) & mask) == val) return 0;
  }

  return -ETIME;
}

static int nvme_setup_prps(struct nvme_dev *dev, u64 *prp2, int total_len,
                           u64 dma_addr) {
  ...
  if (length) dma_addr += (page_size - offset);
  ...
  while (nprps) {
    if (i == ((page_size >> 3) - 1)) {
      *(prp_pool + i) = cpu_to_le64((ulong)prp_pool + page_size);
      i = 0;
      prp_pool += page_size;
    }
  ...
  *prp2 = (ulong)dev->prp_pool;
  ...
  flush_dcache_range((ulong)dev->prp_pool,
                     (ulong)dev->prp_pool + dev->prp_entry_num * sizeof(u64));
...
static int nvme_submit_sync_cmd(struct nvme_queue *nvmeq,
  ...
  if (result) *result = readl(&(nvmeq->cqes[head].result));
...
static int nvme_submit_admin_cmd(struct nvme_dev *dev, struct nvme_command *cmd,
                                 u32 *result) {
  return nvme_submit_sync_cmd(dev->queues[NVME_ADMIN_Q], cmd, result,
                              ADMIN_TIMEOUT);
}
...
static void nvme_init_queue(struct nvme_queue *nvmeq, u16 qid) {
  ...
  nvmeq->cq_phase = 1;
  nvmeq->q_db = &dev->dbs[qid * 2 * dev->db_stride];
  log_info("nvme_init_queue memset\n");
  memset((void *)nvmeq->cqes, 0, NVME_CQ_SIZE(nvmeq->q_depth));
  log_info("nvme_init_queue flush_dcache_range\n");
...
static int nvme_configure_admin_queue(struct nvme_dev *dev) {
  ...
  if (result < 0) {
    log_err("failed to nvme_disable_ctrl\n");
    return result;
  }
  ...
  log_info("nvme_configure_admin_queue write bar\n");

  writel(aqa, &dev->bar->aqa);
  nvme_writeq((ulong)nvmeq->sq_cmds, &dev->bar->asq);
  nvme_writeq((ulong)nvmeq->cqes, &dev->bar->acq);

  log_info("nvme_configure_admin_queue enable_ctrl\n");
  result = nvme_enable_ctrl(dev);
  if (result) goto free_nvmeq;

  nvmeq->cq_vector = 0;

  log_info("nvme_configure_admin_queue nvme_init_queue\n");
  nvme_init_queue(dev->queues[NVME_ADMIN_Q], 0);

  log_info("nvme_configure_admin_queue return\n");
  return result;
...
int nvme_identify(struct nvme_dev *dev, unsigned nsid, unsigned cns,
  ...
  invalidate_dcache_range(dma_addr, dma_addr + sizeof(struct nvme_id_ctrl));

  ret = nvme_submit_admin_cmd(dev, &c, NULL);
  //   log_info("nvme_identify nvme_submit_admin_cmd %d\n", ret);
  if (!ret)
    invalidate_dcache_range(dma_addr, dma_addr + sizeof(struct nvme_id_ctrl));
...
static int nvme_create_queue(struct nvme_queue *nvmeq, int qid) {
  ...
  if (result < 0) goto release_cq;

  result = nvme_alloc_sq(dev, qid, nvmeq);
  if (result < 0) goto release_sq;
...
static void nvme_create_io_queues(struct nvme_dev *dev) {
  ...
  log_info("queue count %d max_qid %d online_queues %d\n", dev->queue_count,
           dev->max_qid, dev->online_queues);

  for (i = dev->queue_count; i <= dev->max_qid; i++)
    if (!nvme_alloc_queue(dev, i, dev->q_depth)) break;

  for (i = dev->online_queues; i <= dev->queue_count - 1; i++)
    if (nvme_create_queue(dev->queues[i], i)) break;
}
...
static int nvme_setup_io_queues(struct nvme_dev *dev) {
  ...
  log_info("nvme_free_queues\n");
  /* Free previously allocated queues */
  nvme_free_queues(dev, nr_io_queues + 1);
  log_info("nvme_create_io_queues\n");
...
//   if (ns_id) *ns_id = ns->ns_id;
//   if (eui64) memcpy(eui64, ns->eui64, sizeof(ns->eui64));
...
static int nvme_blk_probe(struct nvme_ns *ns, struct nvme_dev *ndev,
                          unsigned int ns_id) {
  //   struct nvme_dev *ndev = dev_get_priv(udev->parent);
  //   struct blk_desc *desc = dev_get_uclass_plat(udev);
  //   struct nvme_ns *ns = dev_get_priv(udev);
  printf("probe ns id %d\n", ns_id);
  u8 flbas;
...
static ulong nvme_blk_rw(struct nvme_ns *ns, lbaint_t blknr, lbaint_t blkcnt,
                         void *buffer, bool read) {
  //   struct nvme_ns *ns = dev_get_priv(udev);
  ...
  u64 total_len = blkcnt << ns->lba_shift;
  ...
  flush_dcache_range((unsigned long)buffer, (unsigned long)buffer + total_len);
  ...
  while (total_lbas) {
    ...
    if (nvme_setup_prps(dev, &prp2, lbas << ns->lba_shift, temp_buffer))
      return -EIO;
    ...
    status = nvme_submit_sync_cmd(dev->queues[NVME_IO_Q], &c, NULL, IO_TIMEOUT);
    if (status) break;
  ...
  return (total_len - temp_len) >> ns->lba_shift;
}

ulong nvme_blk_read(struct nvme_ns *udev, lbaint_t blknr, lbaint_t blkcnt,
                    void *buffer) {
  return nvme_blk_rw(udev, blknr, blkcnt, buffer, true);
}

ulong nvme_blk_write(struct nvme_ns *udev, lbaint_t blknr, lbaint_t blkcnt,
                     const void *buffer) {
...
int nvme_init(struct nvme_dev *ndev, struct pci_dev *pdev) {
  ...
  ndev->pdev = pdev;
  ndev->bar = (struct nvme_bar *)pdev->resources[0].start;

  INIT_LIST_HEAD(&ndev->namespaces);
  if (readl(&ndev->bar->csts) == -1) {
    ret = -ENODEV;
    printf("Error: Out of memory! read ndev->bar->csts\n");
    goto free_nvme;
  }
  ...
  if (!ndev->queues) {
    ret = -ENOMEM;
    printf("Error: Out of memory! ndev->queues\n");
    goto free_nvme;
  }
  ...
  if (ret) {
    log_err("failed to nvme_configure_admin_queue\n");
    goto free_queue;
  }
  ...
  if (!ndev->prp_pool) {
    ret = -ENOMEM;
    printf("Error: Out of memory! ndev->prp_pool\n");
    goto free_nvme;
  }
  ndev->prp_entry_num = MAX_PRP_POOL >> 3;

  log_info("nvme_setup_io_queues\n");
  ret = nvme_setup_io_queues(ndev);
  if (ret) {
    log_err("failed to nvme_setup_io_queues\n");
    goto free_queue;
  }

  log_info("nvme_get_info_get_identify\n");
  ...
  for (int i = 1; i <= ndev->nn; i++) {
    ...
    if (nvme_identify(ndev, i, 0, (dma_addr_t)(long)id)) {
      log_err("failed to nvme_identify\n");
      ret = -EIO;
      goto free_id;
    }
    ...
    if (!id->nsze) {
      //   log_info("skip inactive namespace\n");
      continue;
    }

    struct nvme_ns *ns_dev = malloc(sizeof(struct nvme_ns));
    log_info("nvme_blk_probe\n");
    nvme_blk_probe(ns_dev, ndev, i);
    ...
    // ret = blk_create_devicef(udev, "nvme-blk", name, IF_TYPE_NVME, -1, 512,
    // 0,
    //                          &ns_udev);
    // if (ret) goto free_id;
