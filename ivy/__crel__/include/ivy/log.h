...
#define log(_cat, _level, _fmt, _args...)                                    \
  ({                                                                         \
    ...
    if (_LOG_DEBUG != 0 || _l <= _LOG_MAX_LEVEL)                             \
      _log((enum log_category_t)(_cat), (enum log_level_t)(_l | _LOG_DEBUG), \
           __FILE__, __LINE__, __func__, pr_fmt(_fmt), ##_args);             \
  })
  ...
  ({                                                                       \
    ...
    if (_LOG_DEBUG != 0 || _l <= _LOG_MAX_LEVEL)                           \
      _log_buffer((enum log_category_t)(_cat),                             \
                  (enum log_level_t)(_l | _LOG_DEBUG), __FILE__, __LINE__, \
                  __func__, _addr, _data, _width, _count, _linelen);       \
  })
#else
...
#define log(_cat, _level, _fmt, _args...)                                   \
  ({                                                                        \
    int _l = _level;                                                        \
    if (_LOG_DEBUG != 0 || _l <= LOGL_INFO || (_DEBUG && _l == LOGL_DEBUG)) \
      printf(_fmt, ##_args);                                                \
  })
...
#define debug_cond(cond, fmt, args...)                                     \
  ({                                                                       \
    if (cond)                                                              \
      log(LOG_CATEGORY, (enum log_level_t)(LOGL_FORCE_DEBUG | _LOG_DEBUG), \
          fmt, ##args);                                                    \
  })
...
#define warn_non_spl(fmt, args...) debug_cond(!_SPL_BUILD, fmt, ##args)
...
#define assert(x)                                                        \
  ({                                                                     \
    if (!(x) && _DEBUG) __assert_fail(#x, __FILE__, __LINE__, __func__); \
  })
...
#define assert_noisy(x)                                    \
  ({                                                       \
    bool _val = (x);                                       \
    if (!_val) __assert_fail(#x, "?", __LINE__, __func__); \
    _val;                                                  \
  })
...
#define log_ret(_ret)                                                        \
  ({                                                                         \
    int __ret = (_ret);                                                      \
    if (__ret < 0) log(LOG_CATEGORY, LOGL_ERR, "returning err=%d\n", __ret); \
    __ret;                                                                   \
  })
...
#define log_retz(_ret)                                                   \
  ({                                                                     \
    int __ret = (_ret);                                                  \
    if (__ret) log(LOG_CATEGORY, LOGL_ERR, "returning err=%d\n", __ret); \
    __ret;                                                               \
  })
...
//  * system.
//  *
//  * TODO(sjg@chromium.org): Compress this struct down a bit to reduce space,
//  e.g.
...
//  *
//  * Since drivers are set up at build-time we need to have a separate device
//  for
//  * the run-time aspects of drivers (currently just a list of filters to apply
//  * to records send to this device).
//  *
//  * @next_filter_num: Sequence number of next filter filter added (0=no
//  filters *	yet). This increments with each new filter on the device, but
//  never *	decrements
...
//  *
//  * If a message matches all criteria, then it is allowed. If LOGFF_DENY is
//  set,
//  * then it is denied instead.
//  *
//  * @filter_num: Sequence number of this filter.  This is returned when adding
//  a *	new filter, and must be provided when removing a previously added
//  *	filter.
...
// #define LOG_GET_DRIVER(__name) \ 	ll_entry_get(struct log_driver, __name,
// log_driver)
...
//  entries *	long, terminated by %LC_END if fewer
...
// int do_log_test(struct cmd_tbl *cmdtp, int flag, int argc, char *const
// argv[]);

// /**
//  * log_add_filter_flags() - Add a new filter to a log device, specifying
//  flags
//  *
...
//  *	files are permitted
//  * Return:
//  *   the sequence number of the new filter (>=0) if the filter was added, or
//  a
//  *   -ve value on error
//  */
// int log_add_filter_flags(const char *drv_name, enum log_category_t
// cat_list[], 			 enum log_level_t level, const char *file_list,
// int flags);
...
//  *
//  * @drv_name: Driver name to remove the filter from (since each driver only
//  has *	a single device)
...
//  *
//  * Devices are referenced by their driver, so use LOG_GET_DRIVER(name) to
//  pass
...
static inline int log_get_default_format(void) {
  return BIT(LOGF_MSG) | (IS_ENABLED(CONFIG_LOGF_FILE) ? BIT(LOGF_FILE) : 0) |
