...
logger = logging.getLogger('ivy.cmd.app')
...
class PL011:
    ...
    def dump_def(self, f):
        ...
        f.write(
            '#define IVY_DT_STDOUT_PL011_BASE (0x{:x})\n'.format(self._base))
...
class App:
    def __init__(self, dt: dt.DeviceTree, cfg: Config):
        ...
        # map config space and all ranges of all pci host as DEVICE
        # in linux kernel, for aarch64, the memory attribute of ioremap is DEVICE_nGnRE
        for ph in dt.pci_hosts:
            lpt.MapRange(ph.cfg_base, ph.cfg_base,
                         ph.cfg_size, pt.PROT_DEVICE_nGnRE)
            # allow huge pages, MEM64 is very huge, otherwise mapping will be very slow and
            #  the result page table will very big, maybe over 64MB
            if ph.io_size > 0:
                logger.info(
                    f'map pci host io range {ph.io_base:#x}, {ph.io_size:#x}')
                lpt.MapRange(ph.io_base, ph.io_base,
                             ph.io_size, pt.PROT_DEVICE_nGnRE)
            if ph.mem32_size > 0:
                lpt.MapRange(ph.mem32_base, ph.mem32_base,
                             ph.mem32_size, pt.PROT_DEVICE_nGnRE)
            if ph.mem64_size > 0:
                lpt.MapRange(ph.mem64_base, ph.mem64_base,
                             ph.mem64_size, pt.PROT_DEVICE_nGnRE)
        ...
        print('uart: {}@{:x} baudrate {}'.format(stdout_name,
              stdout_drv.base, self._stdout_baudrate))
    ...
    def __GenDT(self):
        with open('ivy_dt.h', 'w') as f:
            ...
            f.write(f'#define IVY_DT_MAX_TEXT_SIZE ({MAX_TEXT_SIZE:#x})\n')

            self._stdout_drv.dump_def(f)
            f.write(
                f'#define IVT_DT_UART_BARD_RATE ({self._stdout_baudrate})\n')
            ...
            f.write(
                f'#define IVY_DT_NUM_RESERVED_MEMORY {len(self._dt.memories_reserved)}\n')
            f.write(
                f'#define IVY_DT_NUM_FREE_MEMORY {len(list(self.FreeRanges()))}\n')
            f.write(f'#define IVY_DT_NUM_PCI_HOST ({len(self._dt.pci_hosts)})')
        ...
        extern struct dt_pci_host ivy_dt_pci_hosts[IVY_DT_NUM_PCI_HOST];
        ...
        with open('ivy_dt.c', 'w') as f:
            ...
            for mem in self._dt.memories:
                f.write('{{.start = 0x{:x}, .size = 0x{:x}, .numa_id = 0x{:x}}},\n'.format(
                    mem.start, mem.size, mem.nid))
            f.write('};\n')

            f.write(
                'dt_memory_t ivy_dt_reserved_memories[IVY_DT_NUM_RESERVED_MEMORY] = {\n')
            ...
            f.write(
                'dt_memory_t ivy_dt_free_memories[IVY_DT_NUM_FREE_MEMORY] = {\n')
            ...
            f.write(
                'struct dt_pci_host ivy_dt_pci_hosts[IVY_DT_NUM_PCI_HOST] = {\n')
            for ph in self._dt.pci_hosts:
                f.write('{')
                f.write(
                    f'.name  = "{ph.name}", .cfg_base = {ph.cfg_base:#x}, .cfg_size = {ph.cfg_size:#x}, ')
                f.write(f'.io_base = {ph.io_base:#x}, .io_size = {ph.io_size:#x}, .mem32_base = {ph.mem32_base:#x}, .mem32_size = {ph.mem32_size:#x}, .mem64_base = {ph.mem64_base:#x}, .mem64_size = {ph.mem64_size:#x}')
                f.write('},')
            f.write('};\n')
    ...
    def __GenPageTable(self):
        ...
        with open('ivy_pt.h', 'w') as f:
            ...
            f.write("#define MAIR_EL1_VAL ({:#x})\n".format(
                self._lpt.mair_el1_val))
            f.write("#define TCR_EL1_VAL ({:#x})\n".format(
                self._lpt.tcr_el1_val))
...
def Main():
    ...
    parser.add_argument(
        "--device_tree", help="device tree source file", type=str, required=True)
    ...
    with open('ivy_app_cfg.py', 'w') as f:
        ...
        for lid, cpu in enumerate(app.cpus):
            f.write(
                f'ivy.cfg.Cpu(id={cpu.id}, lid = {lid}, numa_id={cpu.nid}),')
        f.write(']\n')

        f.write('free_mem_ranges = [')
        for fr in app.FreeRanges():
            f.write(
                f'ivy.cfg.MemoryRange(base={fr[0]:#x}, size={fr[1]-fr[0]:#x}, numa_id={fr[2]:#x}),')
        f.write(']\n')

        f.write(f'device_tree_file = \'{args.device_tree}\'\n')
