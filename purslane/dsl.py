...
class Sequence:
  ...
  def __exit__(self, exc_type, exc_value, traceback):
    self.final_node.AddPredecessor(self.cur_pred)
...
class Parallel:
  ...
  def __exit__(self, exc_type, exc_value, traceback):
    if len(self._sub_scope_finals) > 0:
      for sf in self._sub_scope_finals:
        self.final_node.AddPredecessor(sf)
    else:
  ...
  def SubScopeOver(self, fn):
    self._sub_scope_finals.append(fn)
    # reset pred
    global_ctx.pred = self.init_node
...
def PrepareArgParser(parser: argparse.ArgumentParser) -> None:
  ...
  parser.add_argument('--graph_output', default='graph.json', help='graph output file name')
  parser.add_argument('--num_executors', default=2, type=int, help='number of executors')
  parser.add_argument('--uvm_output', default=None, help='uvm output')
  parser.add_argument('--uvm_pkg_name', default=None, help='uvm package name')
  parser.add_argument('--uvm_executor_name', default='uvm_executor', help='uvm executor name')
  parser.add_argument('--soc_output', default=None, help='soc output file name')
  parser.add_argument('--soc_cooperative', action='store_true', help='soc cooperative')
  parser.add_argument('--soc_cooperative_hosted', action='store_true', help='soc copoerative hosted based on pthread')
...
def Run(act, args: argparse.Namespace) -> None:
  ...
  if args.uvm_output is not None:
    dag.UvmBackendGenF(global_ctx.graph, args.uvm_executor_name, f'{args.uvm_output}', args.uvm_pkg_name)
  
  if args.soc_output is not None:
    if args.soc_cooperative:
      logger.debug(f'soc cooperative hosted: {args.soc_cooperative_hosted}')
      dag.CooperativeCBackendGenF(global_ctx.graph, args.soc_cooperative_hosted, False, f'{args.soc_output}', args.debug)
    else:
      dag.PreemptiveCBackendGenF(global_ctx.graph, True, f'{args.soc_output}')
